...
private:
	...
	int ReadyIndexesDelta = 2147483647;
    int SplitCount = 0;
public:
    Worker()
    {
...
public:
    ...
    int CalculateDelta(int** Indexes, int GroupSize, int Count)
    {
        int Maximal = 0;
        for (int i = 0; i < GroupSize; i++)
        {
            if (Indexes[0][i] - 1 >= 0) {
                Maximal += Numbers[Indexes[0][i] - 1];
            }
        }
        int Minimal = Maximal;
        for (int v = 0; v < Count; v++)
